"use strict";
/* Bell Performance 13.6.0 — athlete profile, application control, First Flight, and Settings experience. */
(function(){
  ...
  function buildCoachControls(){const card=document.createElement("article");card.className="card bell133-coach-control-card";card.innerHTML=`<div class="bell133-card-heading"><div><span class="metric-label">Application Control</span><h3>Choose how Bell behaves</h3><p>Workout Planner follows a stable program and waits for manual changes. Bell Coach adapts training from readiness, progress, and feedback.</p></div></div><div class="bell133-form-grid compact"><label>Application mode<select id="settingsControlMode" onchange="updateControlModeSettingsUI()"><option value="coach">Bell Coach — adaptive</option><option value="planner">Workout Planner — fixed plan</option></select></label><label class="settings-coach-only">Coaching detail<select id="settingsCoachDetail"><option>Concise</option><option>Balanced</option><option>Detailed</option></select></label><label class="settings-coach-only">Formal check-ins<select id="settingsCheckInFrequency"><option>Every session</option><option>Weekly</option><option>Every phase</option></select></label><label class="settings-coach-only">Message style<select id="coachMessageStyle"><option value="Performance">Performance</option><option value="Stoic">Stoic</option><option value="Faith-Based">Faith-Based</option><option value="Mixed">Mixed</option><option value="Off">Off</option></select></label><label class="settings-coach-only" id="scriptureFrequencyWrap">Scripture frequency<select id="scriptureFrequency"><option value="Occasionally">Occasionally</option><option value="Several">A few times per week</option><option value="Daily">Daily</option></select></label></div><div class="application-control-note" id="settingsControlModeNote"></div><button class="secondary" type="button" onclick="saveCoachMessagePreferences()">Save Application Settings</button><div class="hint settings-coach-only">Faith-based messages remain optional and never replace training rationale.</div>`;panel("coach").appendChild(card);}
  ...
  function renderModernSettings(){
    ...
    updateControlModeSettingsUI();const selected=data.settings.lastSettingsPanel||"profile";showSettingsPanel(selected,false);
  }
  ...
  function updateControlModeSettingsUI(){
    ...
    const note=$("settingsControlModeNote");if(note)note.textContent=coachMode?"Bell may automatically adjust volume, load, conditioning, recovery, and scheduling when the athlete's data supports a change.":"Readiness remains visible, but the scheduled workout will not be automatically reduced, replaced, or rescheduled. Use Modify when you want a change.";
