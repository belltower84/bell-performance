...
(function(){
  ...
  function buildDiagnosticsCard(){const card=document.createElement("article");card.className="card bell133-diagnostics-card";card.innerHTML=`<div class="bell133-card-heading"><div><span class="metric-label">System Status</span><h3>Bell Diagnostics</h3><p>Review the local profile schema, readiness inputs, and Bell Core connection before troubleshooting.</p></div></div><div class="bell133-diagnostic-grid"><div><span>App version</span><strong id="settingsAppVersion">${VERSION}</strong></div><div><span>Profile schema</span><strong id="settingsProfileSchema">v1</strong></div><div><span>Bell Core</span><strong id="settingsCoreStatus">Local mode</strong></div><div><span>Last profile update</span><strong id="settingsProfileUpdated">Not yet saved</strong></div></div><div class="bell133-readiness-diagnostics"><span>Readiness calculation</span><div class="bell133-diagnostic-grid"><div><span>Daily check-in</span><strong id="settingsReadinessDaily">—</strong></div><div><span>7-day modifier</span><strong id="settingsReadinessTrend">0</strong></div><div><span>Protective modifier</span><strong id="settingsReadinessProtection">0</strong></div><div><span>Valid feedback records</span><strong id="settingsReadinessFeedback">0</strong></div></div></div>`;panel("app").appendChild(card);}
  ...
  function renderModernSettings(){
    ...
    const readinessDebug=typeof readinessBreakdown==="function"?readinessBreakdown():null;if($("settingsReadinessDaily"))$("settingsReadinessDaily").textContent=readinessDebug?String(readinessDebug.dailyScore):"—";if($("settingsReadinessTrend"))$("settingsReadinessTrend").textContent=readinessDebug?`${readinessDebug.trendModifier>0?"+":""}${readinessDebug.trendModifier}`:"0";if($("settingsReadinessProtection"))$("settingsReadinessProtection").textContent=readinessDebug?`${readinessDebug.protectiveModifier>0?"+":""}${readinessDebug.protectiveModifier}`:"0";if($("settingsReadinessFeedback"))$("settingsReadinessFeedback").textContent=readinessDebug?String(readinessDebug.weekly.feedbackCount):"0";
