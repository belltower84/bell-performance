...
function commandOpenRecoverySession(dateKey=selectedDashboardDateKey()){
  closeCommandTile();
  if(typeof openMobilityRoutine==='function')openMobilityRoutine(dateKey);
}
...
renderPremiumReadiness=function(){
  const breakdown=typeof readinessBreakdown==='function'?readinessBreakdown():{finalScore:readinessScore(),dailyScore:readinessScore(),trendModifier:0,protectiveModifier:0,explanation:''};
  const score=breakdown.finalScore,status=readinessStatus(score),r=data.settings.readiness||{},checkedIn=r.lastPromptDate===todayKey();
  ...
  commandSetText('premiumReadinessScore',checkedIn?score:'—');commandSetText('premiumReadinessStatus',checkedIn?(status==='GREEN'?'READY':status==='YELLOW'?'CAUTION':'RECOVER'):'CHECK IN');commandSetText('premiumReadinessDetail',checkedIn?`${descriptions[status]} ${breakdown.explanation}`:'Complete today’s check-in to personalize your training.');
  commandSetText('premiumSleep',typeof readinessSleepLabel==='function'?readinessSleepLabel(r):premiumSleepDuration(r));commandSetText('premiumBody',typeof readinessBodyLabel==='function'?readinessBodyLabel(r):commandReadinessWord(r.recoveryStatus));commandSetText('premiumEnergy',typeof readinessEnergyLabel==='function'?readinessEnergyLabel(r):commandReadinessWord(r.energy));commandSetText('commandPain',(typeof readinessPainLabel==='function'&&readinessPainLabel(r)==='Review')||data.settings?.injuryProfile?.hasLimitations?'Review':'None');commandSetText('commandTime',commandAvailableTime(r));
  commandSetText('readinessDailyScore',checkedIn?String(breakdown.dailyScore):'—');
  commandSetText('readinessTrendModifier',checkedIn?`${breakdown.trendModifier>0?'+':''}${breakdown.trendModifier}`:'0');
  commandSetText('readinessProtectiveModifier',checkedIn?`${breakdown.protectiveModifier>0?'+':''}${breakdown.protectiveModifier}`:'0');
  commandSetText('readinessScoreExplanation',checkedIn?breakdown.explanation:'Complete today’s check-in to see how Bell calculated readiness.');
  const trendOut=document.getElementById('readinessTrendModifier');if(trendOut)trendOut.dataset.negative=String(breakdown.trendModifier<0);
  const protectionOut=document.getElementById('readinessProtectiveModifier');if(protectionOut)protectionOut.dataset.protective=String(breakdown.protectiveModifier<0);
  const breakdownHost=document.getElementById('commandReadinessBreakdown');if(breakdownHost)breakdownHost.hidden=!checkedIn;
...
renderPremiumMission=function(){
  ...
  if(start){
    ...
    if(!sessions.length){
      const recoveryDone=Boolean(data.mobility?.completedDates?.includes(key));
      start.textContent=recoveryDone?'✓ Recovery Complete':futureDay?'View Recovery':'Start Recovery';
  ...
  if(modify){
    ...
    modify.textContent=sessions.length?'✎ Modify':'Recovery Options';
    modify.onclick=sessions.length?(target&&!futureDay?()=>commandSessionCall(target,'preview'):()=>showScreen('more')):()=>openCommandTile('recovery');
  }
...
function openCommandTile(type){
  ...
  else if(type==='recovery'){const recoveryKey=typeof selectedDashboardDateKey==='function'?selectedDashboardDateKey():todayKey(),recoveryOnly=typeof premiumAllSessions==='function'&&premiumAllSessions().length===0,breakdown=typeof readinessBreakdown==='function'?readinessBreakdown():{dailyScore:score,trendModifier:0,protectiveModifier:0,explanation:''};html=commandDetailStats([{label:'Final readiness',value:`${score}/100`,note:status},{label:'Today’s check-in',value:`${breakdown.dailyScore}/100`},{label:'7-day trend',value:`${breakdown.trendModifier>0?'+':''}${breakdown.trendModifier}`},{label:'Protection',value:`${breakdown.protectiveModifier>0?'+':''}${breakdown.protectiveModifier}`}])+`<div class="command-detail-section"><h3>How Bell calculated it</h3><p>${escapeHtml(breakdown.explanation||document.getElementById('premiumReadinessDetail')?.textContent||'Complete the daily check-in to personalize recovery.')}</p></div><div class="command-detail-section"><h3>Recovery Session</h3><p>${escapeHtml(`${data.mobility?.minutes||10} minute ${typeof resolvedMobilityFocus==='function'?resolvedMobilityFocus():'full-body'} routine matched to today’s training.`)}</p></div>`;title='Recovery';kicker='Readiness & Mobility';actions=[commandAction('Update Check-In',()=>{closeCommandTile();openDailyReadiness();}),commandAction(recoveryOnly?'Start Recovery':'Open Recovery Session',()=>commandOpenRecoverySession(recoveryKey),true)];}
