...
function inferExerciseMeta(name){
  ...
  else if(/bench|chest press|\b(?:flat|incline|decline)?\s*db press\b|push-up|push up|floor press|fly|pec deck|crossover/.test(n)){pattern="Push";primary=["Chest","Triceps"];secondary=["Front Delts"];equipment=[/dumbbell/.test(n)?"Dumbbell":/cable|crossover/.test(n)?"Cable":/machine|pec deck/.test(n)?"Machine":/push/.test(n)?"Bodyweight":"Barbell and Bench"];role=["strength","hypertrophy"]}
