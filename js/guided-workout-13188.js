  ...
  window.renderActiveWorkout=function(){
    ...
    c.innerHTML=`<section class="gw-shell">
      ...
      <footer class="gw-bottom-dock">
        <div class="gw-next-lock ${upcoming?'':'is-finish'}"><span class="gw-lock-icon">${state.rating?'○':'🔒'}</span><div><small>${upcoming?'Next exercise':'After this exercise'}</small><strong>${esc(upcoming?.name||'Session review')}</strong></div></div>
        ${state.rating?`<div class="gw-dock-hint">Rate the load above to unlock the next exercise.</div>`:`<div class="gw-dock-actions"><button class="gw-skip" type="button" onclick="gwSkipSet(${exerciseIndex},${firstOpen})">Skip ${openLabel}</button><button class="gw-next-set" type="button" onclick="gwNextSet(${exerciseIndex})"><span>Next set</span><small>Uncompleted set will be marked skipped</small></button></div>`}
      </footer>
    </section>`;
