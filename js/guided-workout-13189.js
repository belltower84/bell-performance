(function(){
  ...
  function muscleProfile(name){
    ...
    if(/pull-up|pulldown|row|back|chin/.test(n))return['Lats','Upper back','Biceps'];
    if(/squat|leg press|lunge|split squat/.test(n))return['Quads','Glutes','Core'];
  ...
  window.renderActiveWorkout=function(){
    ...
    const rating=state.rating?`<section class="gw-rating"><div><div class="gw-kicker">Exercise feedback</div><h3>How did the suggested weight feel?</h3><p>Rate the load once all sets are completed or skipped.</p></div><div class="gw-rating-actions"><button onclick="gwRateExercise(${exerciseIndex},'too_hard')">Too hard</button><button class="is-primary" onclick="gwRateExercise(${exerciseIndex},'just_right')">Just right</button><button onclick="gwRateExercise(${exerciseIndex},'too_light')">Too light</button></div></section>`:'';
    ...
    c.innerHTML=`<section class="gw-shell">
      ...
      <footer class="gw-bottom-dock">
        <div class="gw-next-lock ${upcoming?'':'is-finish'}"><span class="gw-lock-icon">${state.rating?'○':'🔒'}</span><div><small>${upcoming?'Next exercise · locked':'After this exercise'}</small><strong>${esc(upcoming?.name||'Session review')}</strong></div></div>
        ${state.rating?`<div class="gw-dock-hint">Rate the load above to unlock the next exercise.</div>`:`<div class="gw-dock-actions"><button class="gw-skip" type="button" onclick="gwSkipSet(${exerciseIndex},${firstOpen})">Skip set</button><button class="gw-next-set" type="button" onclick="gwNextSet(${exerciseIndex})"><span>Next</span><small>If not completed, this set is logged as skipped</small></button></div>`}
      </footer>
    </section>`;
