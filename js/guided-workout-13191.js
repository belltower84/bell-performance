(function(){
  ...
  function navigateExercise(delta){
    ...
    if(target>current){
      const exercise=active.exercises[current];
      if(Array.isArray(exercise?.sets))exercise.sets.forEach(set=>{if(!resolved(set)){set.done=false;set.skipped=true;set.skippedAt=new Date().toISOString();}});
    }
