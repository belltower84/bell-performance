(function(){
  ...
  window.renderActiveWorkout=function(){
    ...
    const rows=exercise.sets.map((set,setIndex)=>{
      ...
      const stateLabel=set.done?'Completed':set.skipped?'Skipped':'Complete';
      const stateClass=set.done?'is-done':set.skipped?'is-skipped':'';
      const primaryAction=set.done||set.skipped?`gwSetSetState(${exerciseIndex},${setIndex},'reset')`:`gwSetSetState(${exerciseIndex},${setIndex},'complete')`;
      return `<div class="gw-set-row ${set.done?'is-done':''} ${set.skipped?'is-skipped':''} ${current?'is-current':''}">
        ...
        <div class="gw-result-cell"><button class="gw-result-button ${stateClass}" type="button" onclick="${primaryAction}">${stateLabel}</button>${!resolved(set)?`<button class="gw-skip-link" type="button" onclick="gwSetSetState(${exerciseIndex},${setIndex},'skip')">Skip</button>`:''}</div>
      </div>`;
    }).join('');
    ...
    c.innerHTML=`<section class="gw-shell">
      ...
      <header class="gw-sticky-head"><div class="gw-head-top"><div><div class="gw-kicker">Exercise ${exerciseIndex+1} of ${active.exercises.length}</div><h2>${esc(exercise.name||'Current exercise')}</h2></div><div class="gw-head-actions"><button type="button" onclick="closeWorkout?.()">Save & Exit</button><button class="is-danger" type="button" onclick="discardWorkout?.()">Discard</button></div></div><div class="gw-head-meta"><span>${exerciseHandled}/${exercise.sets.length} handled</span></div><div class="gw-muscles">${muscleIcon()}<div>${muscles.map(m=>`<span>${esc(m)}</span>`).join('')}</div></div></header>
      <article class="gw-current">
        ${exercise.cue?`<div class="gw-sub">${esc(exercise.cue)}</div>`:''}
        <div class="gw-set-head" aria-hidden="true"><span>Set</span><span>Weight</span><span>Reps</span><span>Result</span></div>
        <div class="gw-set-list">${rows}</div>
        <details class="gw-more"><summary>Exercise options</summary><div class="gw-tools"><button type="button" onclick="openExerciseGuide?.('${esc(exercise.name).replace(/'/g,'&#39;')}')">Exercise guide</button><button type="button" onclick="openExerciseReplacement?.(${exerciseIndex})">Replace exercise</button><button type="button" onclick="document.getElementById('sessionNotes')?.focus()">Session notes</button></div></details>
      </article>
      <div class="gw-dock-spacer" aria-hidden="true"></div>
      <footer class="gw-bottom-dock">
        ${state.rating?`<div class="gw-rating-dock"><div class="gw-rating-copy"><small>Exercise feedback</small><strong>How did the weight feel?</strong></div><button onclick="gwRateExercise(${exerciseIndex},'too_hard')">Too hard</button><button class="is-primary" onclick="gwRateExercise(${exerciseIndex},'just_right')">Just right</button><button onclick="gwRateExercise(${exerciseIndex},'too_light')">Too light</button></div>`:`<div class="gw-dock-top"><div class="gw-exercise-nav"><button type="button" ${exerciseIndex===0?'disabled':''} onclick="gwNavigateExercise(-1)">‹ Previous</button><div class="gw-next-lock ${upcoming?'':'is-finish'}"><small>${upcoming?'Next exercise':'After this exercise'}</small><strong>${esc(upcoming?.name||'Session review')}</strong></div><button class="is-primary" type="button" ${exerciseIndex>=active.exercises.length-1?'disabled':''} onclick="gwNavigateExercise(1)">Next ›</button></div><div class="gw-rest-slot"></div></div>`}
      </footer>
    </section>`;
