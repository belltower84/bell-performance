(function(){
  ...
  function syncWorkoutDockHeight(){
    const modal=document.querySelector('#workoutModal .modal-box.gw-stable-13202');
  ...
  function observeWorkoutDock(){
    const dock=document.querySelector('#workoutModal .gw-stable-13202 .gw-action-dock');
    if(!dock)return;
  ...
  window.renderActiveWorkout=function(){
    const c=document.getElementById('activeExercises');if(!c)return;
    document.querySelector('#workoutModal .modal-box')?.classList.add('gw-execution-active','gw-commercial','gw-stable-13202');
    ...
    c.innerHTML=`<section class="gw-commercial-shell" onclick="if(event.target.closest('.gw-session-menu')===null&&!event.target.closest('.gw-menu-trigger'))gwCloseSessionMenu()">
      ...
      <section class="gw-feel ${feelReady?'is-ready':''}"><div class="gw-feel-title">HOW DID THE WEIGHT FEEL? <span>ⓘ</span></div><div class="gw-feel-buttons"><button onclick="gwRateExercise(${exerciseIndex},'too_hard')">⌄ TOO HARD</button><button class="right" onclick="gwRateExercise(${exerciseIndex},'just_right')">✓ JUST RIGHT</button><button onclick="gwRateExercise(${exerciseIndex},'too_light')">⌃ TOO LIGHT</button></div></section>
