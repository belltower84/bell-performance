...
function clearHowToTarget(){if(activeTourTarget){activeTourTarget.classList.remove("tour-highlight");activeTourTarget=null;}document.body.classList.remove("tour-active");}
...
function closeHowToGuide(){
  ...
  if(resumeFirstFlightAfterTour){
    ...
    window.setTimeout(()=>openFirstFlight(1),180);
  }
}
...
function startHowToAutoplay(){document.getElementById("howToAutoPlay").textContent="Ⅱ Pause Slideshow";howToTimer=setInterval(()=>{if(howToIndex===howToSlides.length-1){stopHowToAutoplay();return;}howToIndex++;renderHowToSlide();},6500);}
