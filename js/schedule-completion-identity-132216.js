"use strict";
/* Bell Performance 13.22.16 — date-bound completion identity after schedule changes. */
(function(){
  ...
  window.bellHistoryCompletesPlannedSession=function(item,sessionKey,mission,scheduledDate){
    ...
    const itemSessions=typeof sessionsFromPlanItem==="function"?sessionsFromPlanItem(item):[];
    const sameTypeCount=itemSessions.filter(s=>missionType(s.mission)===targetType).length;
