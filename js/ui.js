...
/* Bell Performance 8.7.0 — streamlined First Flight */
...
function bpPopulateSecondary(){const el=byId("onboardingSecondaryEmphasis");if(!el)return;const old=el.value;el.innerHTML=(BP_PRIMARY_OPTIONS[bpPrimaryGoal()]||BP_PRIMARY_OPTIONS.Hybrid).map(x=>`<option value="${x}">${x}</option>`).join("");if([...el.options].some(o=>o.value===old))el.value=old;bpToggleSecondaryDate();}
...
openFirstFlight=function(startStep=null){const modal=byId("onboardingModal");if(!modal||!modal.classList.contains("hidden"))return;byId("onboardingAthleteName").value=data.settings.athleteName||"";byId("onboardingAge").value=data.nutrition.age||"";byId("onboardingSex").value=data.settings.sex||"Prefer not to say";byId("onboardingBodyweight").value=data.settings.weight||"";const h=Number(data.nutrition.height)||0;byId("onboardingHeightFeet").value=h?Math.floor(h/12):"";byId("onboardingHeightInches").value=h?h%12:"";byId("onboardingMessageStyle").value=data.settings.coachMessages?.style||"Performance";byId("onboardingScriptureFrequency").value=data.settings.coachMessages?.scriptureFrequency||"Occasionally";bpSetRadio("onboardingPrimaryGoal",bpPrimaryFromSaved());bpPopulateSecondary();const savedSecondary=data.settings?.secondaryTrainingGoal||"Balanced Program";if([...byId("onboardingSecondaryEmphasis").options].some(o=>o.value===savedSecondary))byId("onboardingSecondaryEmphasis").value=savedSecondary;byId("onboardingSecondaryTargetDate").value=data.settings?.secondaryTargetDate||"";bpSetRadio("onboardingExperience",data.settings?.trainingExperience||"Intermediate");loadOnboardingInjuryProfile();onboardingStep=Math.max(0,Math.min(5,startStep===null?0:Number(startStep)));renderOnboardingStep();modal.classList.remove("hidden");document.body.classList.add("modal-open");};
renderOnboardingStep=function(){document.querySelectorAll("[data-onboarding-step]").forEach((step,index)=>step.classList.toggle("active",index===onboardingStep));byId("onboardingStepNumber").textContent=String(onboardingStep+1);byId("onboardingProgress").style.width=`${((onboardingStep+1)/6)*100}%`;byId("onboardingBack").disabled=onboardingStep===0;byId("onboardingNext").textContent=onboardingStep===5?(missionEditorActive?"Apply Mission & Rebuild Block":"Launch Bell Performance"):"Continue";const subtitles=["Start with only the essentials.","Choose the training identity that drives your program.","Add an optional emphasis and set your practical schedule.","Set training experience and any current limitations.","Choose how the coach communicates.","Confirm your first mission."];byId("onboardingStepSubtitle").textContent=subtitles[onboardingStep];if(onboardingStep===2)bpPopulateSecondary();if(onboardingStep===5)renderOnboardingReview();};
saveFirstFlightProfile=function(){const name=byId("onboardingAthleteName").value.trim();if(!name){alert("Enter your first name to continue.");byId("onboardingAthleteName").focus();return false;}data.settings.athleteName=name;data.settings.sex=byId("onboardingSex").value||"Prefer not to say";const age=Number(byId("onboardingAge").value);if(age>=8&&age<=100)data.nutrition.age=age;else if(!data.nutrition.age)data.nutrition.age=35;const weight=Number(byId("onboardingBodyweight").value);if(weight>=50&&weight<=700)data.settings.weight=weight;else if(!data.settings.weight)data.settings.weight=180;const feet=Number(byId("onboardingHeightFeet").value),inches=Number(byId("onboardingHeightInches").value);if(feet>=3&&feet<=7&&inches>=0&&inches<=11)data.nutrition.height=feet*12+inches;else if(!data.nutrition.height)data.nutrition.height=68;return true;};
...
validateOnboardingStep=function(){if(onboardingStep===0)return saveFirstFlightProfile();if(onboardingStep===1)return Boolean(bpPrimaryGoal());if(onboardingStep===2){bpApplyEnvironment();return saveOnboardingDualGoals(false);}if(onboardingStep===3)return saveOnboardingInjuryProfile();return true;};
