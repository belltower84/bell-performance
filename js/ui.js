...
function renderWeeklyScheduleStrip() {
  ...
  host.innerHTML=days.map((day,index)=>{
    ...
    const codes=types.length?types.map(type=>`<i class="schedule-code ${type}" title="${type==='strength'?'Strength':'Engine'}">${type==='strength'?'S':'<img src="./assets/energy-running-shoe.svg?v=8610" alt="">'}</i>`).join(""):`<i class="schedule-code rest" title="Rest / recovery">R</i>`;
