...
function renderDashboard() {
  ...
  const plan = typeof dashboardPrimaryPlanItem==="function" ? dashboardPrimaryPlanItem() : currentPlan();
...
function beginPlannedWorkout(planId,sessionKey,mission){const item=(data.plan||[]).find(x=>x.id===planId);const scheduledDate=typeof bellCanonicalPlanDateKey==='function'?bellCanonicalPlanDateKey(item,data.trainingBlock,Number(data.trainingBlock?.currentWeek||1)):(item?.scheduledDate||selectedDashboardDateKey());const sessions=typeof premiumAllSessions==='function'?premiumAllSessions():sessionsFromPlanItem(item);const budget=typeof bellDailySessionBudget==='function'?bellDailySessionBudget(sessions,scheduledDate):null;const allocation=budget?.sessions?.find(x=>x.sessionKey===sessionKey);beginWorkout(mission,{planId,sessionKey,scheduledDate,prescribedDuration:allocation?.minutes});}
...
function renderTodayTrainingCards(){
  ...
  const sessions=dashboardSessionsForToday(),strengthCard=byId('strengthTrainingCard'),engineCard=byId('engineTrainingCard'),option=byId('singleSessionOption'),empty=byId('noTrainingForDay');
  ...
  strengthCard?.classList.toggle('hidden',!strength);engineCard?.classList.toggle('hidden',!engine);empty?.classList.toggle('hidden',sessions.length>0);
  if(!sessions.length){const item=dashboardPrimaryPlanItem(),allCompleted=item&&sessionsFromPlanItem(item).length&&sessionsFromPlanItem(item).every(x=>x.completed);setText('noTrainingDayTitle',allCompleted?'All sessions complete':'No prescribed training');setText('noTrainingDayDetail',allCompleted?'This training day is complete. Recovery and daily standards remain available.':'Recovery, mobility, and daily standards remain available. The dashboard will not pull a session forward from another day.');}
  if(strength){const t=scaledTemplate(strength.mission);setText('todayMission',strength.label||t?.label||strength.mission);setText('todayDuration',t?`${t.duration} minutes`:'—');const button=strengthCard?.querySelector('button');if(button){button.textContent=data.activeWorkout?.planSessionKey===strength.sessionKey?'Resume Strength':'Start Strength';button.setAttribute('onclick',`beginPlannedWorkout('${strength.planId}','${strength.sessionKey}','${strength.mission.replaceAll("'","\\'")}')`);}}
  if(engine){const t=scaledTemplate(engine.mission);setText('engineSessionTitle',engine.label||t?.label||engine.mission);setText('engineSessionPurpose',engine.detail||'Complete the prescribed conditioning session.');const canonicalDuration=Number(engine.prescribedDuration)||Number(t?.duration)||30;const meta=engineCard?.querySelector('.training-meta');if(meta)meta.innerHTML=`◷ <span>${canonicalDuration} minutes</span>`;const button=engineCard?.querySelector('button');if(button){button.textContent=data.activeWorkout?.planSessionKey===engine.sessionKey?'Resume Engine':'Start Engine';button.setAttribute('onclick',`beginPlannedWorkout('${engine.planId}','${engine.sessionKey}','${engine.mission.replaceAll("'","\\'")}')`);}}
  option?.classList.toggle('hidden',sessions.length!==1);if(option&&sessions.length===1)option.innerHTML=optionalSessionHtml(strength?'strength':'engine');
}
...
/* Bell Performance 8.7.1 — streamlined First Flight */
const BP_PRIMARY_OPTIONS={
  "Athletic Performance":["Balanced Program","Improve Speed","Improve Agility","Increase Vertical Jump","Build Power","Improve Conditioning","Sport-Specific Performance"],
  "Hybrid":["Balanced Program","Improve 5K Time","Improve 10K Time","Half Marathon","Marathon","Improve Aerobic Capacity","Build More Strength","Fat Loss","Muscle Gain"],
  "Bodybuilding":["Balanced Program","Fat Loss","Muscle Gain","Body Recomposition","Bring Up Shoulders & Arms","Bring Up Chest & Back","Bring Up Legs","Bring Up Glutes & Hamstrings"],
  "Powerlifting":["Balanced Program","Increase Squat","Increase Bench Press","Increase Deadlift","Meet Preparation"],
  "Olympic Weightlifting":["Balanced Program","Improve Snatch","Improve Clean & Jerk","Technique Development","Competition Preparation"],
  "Functional Fitness":["Balanced Program","Improve Engine","Improve Strength","Improve Gymnastics","Competition Preparation"],
  "Tactical Athlete":["Balanced Program","Improve Run Time","Improve Ruck Performance","Improve Work Capacity","Build Strength","Academy / PT Test Preparation","Selection Preparation"]
};
function bpPrimaryGoal(){return document.querySelector('input[name="onboardingPrimaryGoal"]:checked')?.value||"Hybrid";}
function bpExperience(){return document.querySelector('input[name="onboardingExperience"]:checked')?.value||"Intermediate";}
function bpEnvironment(){return document.querySelector('input[name="onboardingSimpleEnvironment"]:checked')?.value||"commercial";}
function bpPopulateSecondary(preferredValue=null){const el=byId("onboardingSecondaryEmphasis"),grid=byId("onboardingSecondaryGoalGrid");if(!el||!grid)return;const choices=BP_PRIMARY_OPTIONS[bpPrimaryGoal()]||BP_PRIMARY_OPTIONS.Hybrid;const current=preferredValue||el.value||data.settings?.secondaryTrainingGoal||"Balanced Program";el.innerHTML=choices.map(x=>`<option value="${escapeHtml(x)}">${escapeHtml(x)}</option>`).join("");const selected=choices.includes(current)?current:choices[0];el.value=selected;grid.innerHTML=choices.map((x,index)=>`<label class="secondary-goal-option"><input type="radio" name="onboardingSecondaryGoalChoice" value="${escapeHtml(x)}" ${x===selected?"checked":""}/><span>${escapeHtml(x)}</span></label>`).join("");bpToggleSecondaryDate();}
function bpToggleSecondaryDate(){const value=byId("onboardingSecondaryEmphasis")?.value||"Balanced Program";const dated=/5K|10K|Half Marathon|Marathon|Meet|Competition|Test|Selection/i.test(value);byId("onboardingSecondaryDateWrap")?.classList.toggle("hidden",!dated);}
function bpSetRadio(name,value){const el=[...document.querySelectorAll(`input[name="${name}"]`)].find(x=>x.value===value);if(el)el.checked=true;}
function bpPrimaryFromSaved(){const saved=data.settings?.primaryTrainingIdentity;if(saved&&BP_PRIMARY_OPTIONS[saved])return saved;const s=String(data.trainingBlock?.dualGoals?.strengthGoal||"");const a=String(data.settings?.athleteMode||"");if(/Bodybuilding/i.test(s))return"Bodybuilding";if(/Powerlifting/i.test(s))return"Powerlifting";if(/Olympic/i.test(s))return"Olympic Weightlifting";if(/Tactical/i.test(a))return"Tactical Athlete";if(/Functional/i.test(a))return"Functional Fitness";if(/Athlete/i.test(s))return"Athletic Performance";return"Hybrid";}
function bpMissionMapping(primary,secondary){
  const map={
    "Athletic Performance":{strength:"Athlete",mode:"Sprint / Field",goal:"Field Conditioning",athlete:"Hybrid Athlete",sd:4,ed:2},
    "Hybrid":{strength:"Hybrid",mode:"Running",goal:"General Aerobic Fitness",athlete:"Hybrid Athlete",sd:3,ed:3},
    "Bodybuilding":{strength:"Bodybuilding",mode:"General Conditioning",goal:"General Aerobic Fitness",athlete:"Strength Athlete",sd:4,ed:2},
    "Powerlifting":{strength:"Powerlifting",mode:"None / Recovery Only",goal:"Recovery and Mobility",athlete:"Strength Athlete",sd:4,ed:1},
    "Olympic Weightlifting":{strength:"Olympic Lifting",mode:"General Conditioning",goal:"General Aerobic Fitness",athlete:"Strength Athlete",sd:4,ed:2},
    "Functional Fitness":{strength:"Hybrid",mode:"General Conditioning",goal:"Work Capacity",athlete:"Functional Fitness Athlete",sd:4,ed:3},
    "Tactical Athlete":{strength:"Athlete",mode:"Hiking / Rucking",goal:"General Aerobic Fitness",athlete:"Tactical Athlete",sd:4,ed:3}
  };const r={...(map[primary]||map.Hybrid)};
  if(/5K/i.test(secondary)){r.mode="Running";r.goal="5K";r.ed=3;}else if(/10K/i.test(secondary)){r.mode="Running";r.goal="10K";r.ed=3;}else if(/Half Marathon/i.test(secondary)){r.mode="Running";r.goal="Half Marathon";r.ed=4;}else if(/^Marathon$/i.test(secondary)){r.mode="Running";r.goal="Marathon";r.ed=4;}else if(/Ruck/i.test(secondary)){r.mode="Hiking / Rucking";r.goal="10 Mile Ruck";r.ed=3;}else if(/Engine|Aerobic|Run Time|Conditioning/i.test(secondary)){r.mode="Running";r.goal="General Aerobic Fitness";r.ed=3;}else if(/Fat Loss|Recomposition/i.test(secondary)){r.mode="General Conditioning";r.goal="Work Capacity";r.ed=3;}else if(/Meet Preparation/i.test(secondary)){r.mode="None / Recovery Only";r.goal="Recovery and Mobility";r.ed=1;}return r;
}
function bpApplyEnvironment(){if(typeof initializeOnboardingLocationEditor!=="function")return;initializeOnboardingLocationEditor();const preset=bpEnvironment();if(typeof applyOnboardingEnvironment==="function")applyOnboardingEnvironment(preset);if(onboardingLocations?.[0]){onboardingLocations[0].name=({commercial:"Commercial Gym",home:"Home Gym",minimal:"Minimal Equipment",bodyweight:"Bodyweight"})[preset]||"Primary Gym";onboardingActiveLocationId=onboardingLocations[0].id;}}
function bpUpdateOnboardingReadiness(){const sleep=byId("onboardingSleepDurationMinutes");if(sleep){const minutes=Number(sleep.value)||0;byId("onboardingSleepDurationValue").textContent=`${Math.floor(minutes/60)}h ${String(minutes%60).padStart(2,"0")}m`;}const map={SleepQuality:"sleepQuality",Energy:"energy",RecoveryStatus:"recoveryStatus",Motivation:"motivation",TimeAvailability:"timeAvailability"};Object.entries(map).forEach(([suffix,key])=>{const input=byId(`onboarding${suffix}`),output=byId(`onboarding${suffix}Value`);if(input&&output)output.textContent=`${input.value} · ${typeof readinessSliderLabel==="function"?readinessSliderLabel(key,Number(input.value)):input.value}`;});}
function bpLoadOnboardingReadiness(){const r=data.settings?.readiness||{};const sleep=byId("onboardingSleepDurationMinutes");if(sleep)sleep.value=Math.max(180,Math.min(720,(Number(r.sleepHours)||7)*60+(Number(r.sleepMinutes)||30)));const values={onboardingSleepQuality:r.sleepQuality??4,onboardingEnergy:r.energy??4,onboardingRecoveryStatus:r.recoveryStatus??4,onboardingMotivation:r.motivation??4,onboardingTimeAvailability:r.timeAvailability??3};Object.entries(values).forEach(([id,value])=>{const el=byId(id);if(el)el.value=value;});bpUpdateOnboardingReadiness();}
function bpSaveOnboardingReadiness(){const total=Number(byId("onboardingSleepDurationMinutes")?.value)||450;const entry={sleepHours:Math.floor(total/60),sleepMinutes:total%60,sleepQuality:Number(byId("onboardingSleepQuality")?.value)||4,energy:Number(byId("onboardingEnergy")?.value)||4,recoveryStatus:Number(byId("onboardingRecoveryStatus")?.value)||4,motivation:Number(byId("onboardingMotivation")?.value)||4,timeAvailability:Number(byId("onboardingTimeAvailability")?.value)||3};if(typeof commitReadiness==="function")commitReadiness(entry);else data.settings.readiness={...(data.settings.readiness||{}),...entry,lastPromptDate:todayKey()};return true;}
openFirstFlight=function(startStep=null){const modal=byId("onboardingModal");if(!modal||!modal.classList.contains("hidden"))return;byId("onboardingAthleteName").value=data.settings.athleteName||"";byId("onboardingAge").value=data.nutrition.age||"";byId("onboardingSex").value=data.settings.sex||"Prefer not to say";byId("onboardingBodyweight").value=data.settings.weight||"";const h=Number(data.nutrition.height)||0;byId("onboardingHeightFeet").value=h?Math.floor(h/12):"";byId("onboardingHeightInches").value=h?h%12:"";byId("onboardingMessageStyle").value=data.settings.coachMessages?.style||"Performance";byId("onboardingScriptureFrequency").value=data.settings.coachMessages?.scriptureFrequency||"Occasionally";bpSetRadio("onboardingPrimaryGoal",bpPrimaryFromSaved());const savedSecondary=data.settings?.secondaryTrainingGoal||"Balanced Program";bpPopulateSecondary(savedSecondary);byId("onboardingSecondaryTargetDate").value=data.settings?.secondaryTargetDate||"";bpSetRadio("onboardingExperience",data.settings?.trainingExperience||"Intermediate");loadOnboardingInjuryProfile();bpLoadOnboardingReadiness();onboardingStep=Math.max(0,Math.min(5,startStep===null?0:Number(startStep)));renderOnboardingStep();modal.classList.remove("hidden");document.body.classList.add("modal-open");};
renderOnboardingStep=function(){document.querySelectorAll("[data-onboarding-step]").forEach((step,index)=>step.classList.toggle("active",index===onboardingStep));byId("onboardingStepNumber").textContent=String(onboardingStep+1);byId("onboardingProgress").style.width=`${((onboardingStep+1)/6)*100}%`;byId("onboardingBack").disabled=onboardingStep===0;byId("onboardingNext").textContent=onboardingStep===5?(missionEditorActive?"Apply Mission & Rebuild Block":"Launch Training Block"):"Continue";const subtitles=["Build the athlete profile.","Choose the style that drives your program.","Choose the outcome you want to emphasize.","Set a realistic weekly training schedule.","Establish experience, limitations, and readiness.","Review and launch your training block."];byId("onboardingStepSubtitle").textContent=subtitles[onboardingStep];if(onboardingStep===2&&!byId("onboardingSecondaryGoalGrid")?.children.length)bpPopulateSecondary();if(onboardingStep===4)bpUpdateOnboardingReadiness();if(onboardingStep===5)renderOnboardingReview();};
saveFirstFlightProfile=function(){const name=byId("onboardingAthleteName").value.trim(),age=Number(byId("onboardingAge").value),weight=Number(byId("onboardingBodyweight").value),feet=Number(byId("onboardingHeightFeet").value),inches=Number(byId("onboardingHeightInches").value);if(!name){alert("Enter your first name to continue.");byId("onboardingAthleteName").focus();return false;}if(!(age>=8&&age<=100)){alert("Enter a valid age between 8 and 100.");byId("onboardingAge").focus();return false;}if(!(weight>=50&&weight<=700)){alert("Enter a valid bodyweight between 50 and 700 lb.");byId("onboardingBodyweight").focus();return false;}if(!(feet>=3&&feet<=7&&inches>=0&&inches<=11)){alert("Enter a valid height in feet and inches.");byId("onboardingHeightFeet").focus();return false;}data.settings.athleteName=name;data.settings.sex=byId("onboardingSex").value||"Prefer not to say";data.nutrition.age=age;data.settings.weight=weight;data.nutrition.height=feet*12+inches;return true;};
saveOnboardingInjuryProfile=function(){const has=byId("onboardingHasLimitations").checked,areas=has?checkedValues("onboardingInjuryAreaGrid"):[];data.settings.injuryProfile={...(data.settings.injuryProfile||{}),hasLimitations:has,restrictedPatterns:[],affectedAreas:areas,notes:has?byId("onboardingInjuryNotes").value.trim():"",medicalClearance:false,updatedAt:new Date().toISOString(),startedAt:has?(data.settings.injuryProfile?.startedAt||new Date().toISOString()):"",recoveryHistory:data.settings.injuryProfile?.recoveryHistory||[]};return true;};
saveOnboardingDualGoals=function(buildPlan=true){const primary=bpPrimaryGoal(),secondary=byId("onboardingSecondaryEmphasis").value||"Balanced Program",m=bpMissionMapping(primary,secondary),days=Number(byId("onboardingTrainingDays").value)||5,minutes=Number(byId("onboardingSessionMinutes").value)||60;data.settings.primaryTrainingIdentity=primary;data.settings.secondaryTrainingGoal=secondary;data.settings.secondaryTargetDate=byId("onboardingSecondaryTargetDate").value||"";data.settings.trainingExperience=bpExperience();data.settings.athleteMode=m.athlete;data.trainingBlock={...data.trainingBlock,enabled:true,lengthWeeks:/Marathon/.test(secondary)?20:/Half Marathon|Selection/.test(secondary)?16:12,currentWeek:1,trainingDays:days,strengthDays:Math.min(m.sd,days),runDays:Math.min(m.ed,days),sessionMinutes:minutes,startDate:data.trainingBlock?.startDate||todayKey(),generatedAt:new Date().toISOString(),secondaryGoal:secondary,dualGoals:{...(data.trainingBlock?.dualGoals||{}),strengthGoal:m.strength,engineMode:m.mode,engineGoal:m.goal,trainingCoordination:"Coach Decides",engineSessions:Math.min(m.ed,days),targetValue:0}};data.settings.cardioType=m.mode==="General Conditioning"?"Air Bike":m.mode==="None / Recovery Only"?"Running":m.mode;if(buildPlan&&typeof buildCurrentWeekPlan==="function")buildCurrentWeekPlan();return true;};
validateOnboardingStep=function(){if(onboardingStep===0)return saveFirstFlightProfile();if(onboardingStep===1)return Boolean(bpPrimaryGoal());if(onboardingStep===2)return saveOnboardingDualGoals(false);if(onboardingStep===3){bpApplyEnvironment();return true;}if(onboardingStep===4){if(!saveOnboardingInjuryProfile())return false;return bpSaveOnboardingReadiness();}return true;};
nextOnboardingStep=function(){if(!validateOnboardingStep())return;if(onboardingStep<5){onboardingStep++;renderOnboardingStep();return;}completeOnboarding();};
previousOnboardingStep=function(){if(onboardingStep>0){onboardingStep--;renderOnboardingStep();}};
renderOnboardingReview=function(){const primary=bpPrimaryGoal(),secondary=byId("onboardingSecondaryEmphasis").value||"Balanced Program",days=byId("onboardingTrainingDays").value,minutes=byId("onboardingSessionMinutes").value,env=({commercial:"Commercial Gym",home:"Home Gym",minimal:"Minimal Equipment",bodyweight:"Bodyweight"})[bpEnvironment()]||"Primary Gym";byId("onboardingReview").innerHTML=`<div><span>Athlete</span><strong>${escapeHtml(byId("onboardingAthleteName").value.trim())}</strong><small>${escapeHtml(bpExperience())} • ${escapeHtml(byId("onboardingAge").value)} years • ${escapeHtml(byId("onboardingBodyweight").value)} lb</small></div><div><span>Training style</span><strong>${escapeHtml(primary)}</strong><small>This drives the program philosophy.</small></div><div><span>Training focus</span><strong>${escapeHtml(secondary)}</strong><small>${byId("onboardingSecondaryTargetDate").value?`Target: ${escapeHtml(byId("onboardingSecondaryTargetDate").value)}`:"No required event date"}</small></div><div><span>Schedule</span><strong>${escapeHtml(days)} days per week</strong><small>${escapeHtml(minutes)}-minute sessions • ${escapeHtml(env)}</small></div><div><span>Limitations</span><strong>${byId("onboardingHasLimitations").checked?"Modifications enabled":"None reported"}</strong><small>You can update this anytime.</small></div>`;};
completeOnboarding=function(){if(!saveFirstFlightProfile()||!saveOnboardingDualGoals(false)||!saveOnboardingInjuryProfile()||!bpSaveOnboardingReadiness())return;bpApplyEnvironment();if(typeof saveOnboardingEquipment==="function")saveOnboardingEquipment();data.trainingBlock.startDate=todayKey();data.trainingBlock.currentWeek=1;data.trainingBlock.enabled=true;data.trainingBlock.generatedAt=new Date().toISOString();if(typeof buildCurrentWeekPlan==="function")buildCurrentWeekPlan();data.settings.coachMessages={setupComplete:true,style:byId("onboardingMessageStyle").value,scriptureFrequency:byId("onboardingScriptureFrequency").value};data.settings.firstFlightStage="complete";data.settings.firstFlightTourComplete=true;data.settings.firstBlockLaunchMode="recommended";saveData({render:false});byId("onboardingModal").classList.add("hidden");document.body.classList.remove("modal-open");renderApp();showScreen("home");alert(missionEditorActive?"Mission updated. Your revised block is ready.":"Your first training block is ready.");missionEditorActive=false;};
document.addEventListener("change",e=>{if(e.target?.name==="onboardingPrimaryGoal")bpPopulateSecondary("Balanced Program");if(e.target?.name==="onboardingSecondaryGoalChoice"){const select=byId("onboardingSecondaryEmphasis");if(select)select.value=e.target.value;bpToggleSecondaryDate();}if(e.target?.id==="onboardingSecondaryEmphasis")bpToggleSecondaryDate();if(e.target?.id==="onboardingHasLimitations")toggleOnboardingLimitations();if(e.target?.id==="onboardingMessageStyle")toggleOnboardingScripture();});
document.addEventListener("input",e=>{if(/^onboarding(SleepDurationMinutes|SleepQuality|Energy|RecoveryStatus|Motivation|TimeAvailability)$/.test(e.target?.id||""))bpUpdateOnboardingReadiness();});
