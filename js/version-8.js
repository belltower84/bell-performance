...
function planWeekStartKey(){const base=data.trainingBlock?.startDate||localDateKey();const monday=mondayKeyFor(base);const weekOffset=Math.max(0,(Number(data.trainingBlock?.currentWeek)||1)-1);return addLocalDays(monday,weekOffset*7);}
...
// 8.0.4: Optional rotating Core sessions. These are logged but never block prescribed-day completion.
