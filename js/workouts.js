...
function beginWorkout(name) {
  ...
  const planned=data.plan?.find(item=>!item.done&&(item.mission===name||item.secondaryMission===name));
...
function completeWorkout(){if(!data.activeWorkout)return;persistActive();if(data.activeWorkout.engineMetrics&&!parseEngineTime(data.activeWorkout.engineMetrics.manualTime)){data.activeWorkout.engineMetrics.manualTime=formatEngineTime(data.activeWorkout.elapsed||0);data.activeWorkout.officialElapsed=data.activeWorkout.elapsed||0;}const completed={...data.activeWorkout,completedAt:new Date().toISOString()};if(typeof applyCompletedWorkoutProgression==="function")applyCompletedWorkoutProgression(completed);data.history.unshift(completed);const plan=data.plan.find(item=>item.mission===completed.name&&!item.done&&!["skipped","replaced"].includes(item.status));if(plan){plan.done=true;plan.status="completed";plan.completedAt=completed.completedAt;}closeWorkout();data.activeWorkout=null;data.pendingFeedbackSessionId=completed.completedAt;saveData();openPendingSessionFeedback();}
