...
function activeWorkoutElapsed(){const active=data.activeWorkout;if(!active)return 0;const hasAccumulated=active.timerAccumulatedSeconds!==undefined&&active.timerAccumulatedSeconds!==null&&active.timerAccumulatedSeconds!=="";const accumulated=hasAccumulated?Number(active.timerAccumulatedSeconds)||0:Number(active.elapsed)||0;if(!active.timerRunning||!active.timerStartedAt)return Math.max(0,Math.floor(accumulated));const startedAt=new Date(active.timerStartedAt).getTime();if(!Number.isFinite(startedAt))return Math.max(0,Math.floor(accumulated));const delta=Math.max(0,Math.floor((Date.now()-startedAt)/1000));return Math.max(0,Math.floor(accumulated+delta));}
