...
function beginWorkout(name, context={}) {
  ...
  const plannedDuration=Number(context.prescribedDuration)||(planned?.mission===name?planned?.prescribedDuration:planned?.secondaryDuration);
...
function renderActiveWorkout() {
  ...
  active.exercises.forEach((exercise,exerciseIndex)=>{
    ...
    const card=document.createElement("div"); card.className=`exercise-card${exercise.injuryAdjusted?" injury-adjusted":""}`;
    const isEngine=Boolean(active.cardioType)||String(active.name||"").startsWith("R-");
    const feedbackHtml=isEngine
      ? `<div class="exercise-feedback"><span>How did this conditioning segment feel?</span><div class="feedback-buttons"><button class="${exercise.feedback==='below'?'selected':''}" onclick="recordExerciseFeedback(${exerciseIndex},'below')">Below Target</button><button class="${exercise.feedback==='target'?'selected':''}" onclick="recordExerciseFeedback(${exerciseIndex},'target')">On Target</button><button class="${exercise.feedback==='hard'?'selected':''}" onclick="recordExerciseFeedback(${exerciseIndex},'hard')">Too Hard</button><button class="issue ${exercise.feedback==='symptoms'?'selected':''}" onclick="recordExerciseFeedback(${exerciseIndex},'symptoms')">Pain / Symptoms</button></div><div class="feedback-result">${exercise.feedback ? 'Saved for future conditioning progression.' : 'Rate effort, breathing, and control after the segment.'}</div></div>`
      : `<div class="exercise-feedback"><span>How did this exercise feel?</span><div class="feedback-buttons"><button class="${exercise.feedback==='easy'?'selected':''}" onclick="recordExerciseFeedback(${exerciseIndex},'easy')">Too Easy</button><button class="${exercise.feedback==='right'?'selected':''}" onclick="recordExerciseFeedback(${exerciseIndex},'right')">Just Right</button><button class="${exercise.feedback==='heavy'?'selected':''}" onclick="recordExerciseFeedback(${exerciseIndex},'heavy')">Too Heavy</button><button class="issue ${exercise.feedback==='pain'?'selected':''}" onclick="recordExerciseFeedback(${exerciseIndex},'pain')">Pain / Technique</button></div><div class="feedback-result">${exercise.feedback ? 'Saved for next-session progression.' : 'Rate after the final working set.'}</div></div>`;
    card.innerHTML=`<div class="exercise-head"><div class="exercise-num">${exerciseIndex+1}</div><div class="grow"><div class="exercise-name-row"><div class="exercise-name">${exercise.name}</div><div class="exercise-card-actions"><button type="button" onclick="openExerciseDetail(\'${String(exercise.name).replace(/\'/g,"\\\'")}\')">Learn</button>${isEngine?"":`<button type="button" onclick="openExerciseSwap(${exerciseIndex})">Replace</button>`}</div></div><div class="prescription">${exercise.prescription}</div><div class="method-chip">${isEngine?"Conditioning target":(exercise.methodology||"Progressive overload")}</div>${exercise.advancedTechnique?`<div class="bell-technique"><strong>${exercise.advancedTechnique.name}:</strong> ${exercise.advancedTechnique.short}</div>`:""}<div class="cue">${exercise.cue||""}</div>${exercise.injuryAdjusted?`<span class="injury-adjustment-note">Movement adjusted from ${exercise.originalExercise||"the original prescription"} based on the saved limitation profile.</span>`:""}<div class="cue"><strong>Readiness:</strong> ${exercise.scaleNote||"As written"}</div>${isEngine?"":`<div class="cue"><strong>Starting weight:</strong> ${exercise.recommendationDisplay||"Choose by effort"}</div><div class="cue">${exercise.recommendationNote||""}</div>`}</div></div><div class="set-head"><span>${isEngine?"Round":"Set"}</span><span>${isEngine?"Target":"Weight"}</span><span>${isEngine?"Result":"Reps"}</span><span>Done</span></div><div id="sets-${exerciseIndex}"></div>${feedbackHtml}`;
...
function persistActive(){if(!data.activeWorkout)return;data.activeWorkout.rpe=document.getElementById("sessionRpe").value;data.activeWorkout.notes=document.getElementById("sessionNotes").value;if(data.activeWorkout.engineMetrics){const manual=parseEngineTime(data.activeWorkout.engineMetrics.manualTime);data.activeWorkout.officialElapsed=manual||data.activeWorkout.elapsed||0;data.activeWorkout.engineMetrics.pace=calculateEnginePace(data.activeWorkout.engineMetrics,data.activeWorkout.officialElapsed);}saveData({render:false});}
...
function workoutTemplatePreview(name, context={}){
  const template=scaledTemplate(name);if(!template)return null;
  const planned=context.planId?data.plan?.find(item=>item.id===context.planId):data.plan?.find(item=>!item.done&&(item.mission===name||item.secondaryMission===name));
  const plannedDuration=planned?.mission===name?planned?.prescribedDuration:planned?.secondaryDuration;
  return {name,label:template.label||name,duration:Math.max(10,Number(plannedDuration)||Number(template.duration)||30),exercises:template.exercises||[],coachBrief:template.coachBrief||'Execute the prescribed work with deliberate technique and controlled effort.'};
}
function previewPlannedWorkout(planId,sessionKey,name){
  const workout=workoutTemplatePreview(name,{planId,sessionKey});openWorkoutPreview(workout,()=>{closeWorkoutPreview();beginPlannedWorkout(planId,sessionKey,name);});
}
...
function openWorkoutPreview(workout,onBegin){
  ...
  content.innerHTML=`${warmups.length?`<h3>Warm-Up</h3>${rows(warmups)}`:''}<h3>Main Training</h3>${rows(main)}`;
...
function renderWorkoutStage(){
  ...
  const briefing=document.getElementById('missionBriefing'),briefActions=document.getElementById('workoutBriefActions'),warm=document.getElementById('warmupPanel'),warmActions=document.getElementById('warmupActions'),control=document.getElementById('workoutControlCard'),exercises=document.getElementById('activeExercises'),completion=document.getElementById('workoutCompletionCard');
  briefing?.classList.toggle('hidden',stage!=='briefing');briefActions?.classList.toggle('hidden',stage!=='briefing');
